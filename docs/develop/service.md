...
首先，在 services/api.js 文件中设置  API_PROXY_PREFIX.fwq 常量，BASE_URL 像下面这样设置：
```js {2,4}
//跨域代理前缀
const  API_PROXY_PREFIX.fwq='/api'
//base url
...
model.exports = {
  devServer: {
    proxy: {
      '/api': {               //此处要与 /services/api.js 中的  API_PROXY_PREFIX.fwq 值保持一致
