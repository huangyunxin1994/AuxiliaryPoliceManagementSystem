...
export default {
  ...
  methods: {
    ...
    copyCode () {
      ...
      clipboard.on('success', () => {
        this.$message.success(`复制成功，覆盖文件 src/config/config.js 然后重启项目即可生效`).then(() => {
          const localConfig = sessionStorage.getItem(process.env.VUE_APP_SETTING_KEY)
          if (localConfig) {
    ...
    saveSetting() {
      ...
      sessionStorage.setItem(process.env.VUE_APP_SETTING_KEY, JSON.stringify(config))
    ...
    resetSetting() {
      this.$confirm({
        ...
        onOk() {
          sessionStorage.removeItem(process.env.VUE_APP_SETTING_KEY)
